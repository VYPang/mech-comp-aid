...
export const TUTORIAL_SECTIONS = [
  ...
  },
  {
    ...
    body: `
      ...
      <p>When data contain noise, we usually do not ask for a curve that passes through every point. Instead, we look for the curve that best fits the data according to an error measure such as the mean squared error <code>L = (1/N) Σ (ŷᵢ − yᵢ)²</code>. Finding a best-fit curve means finding the parameters that make this loss as small as possible.</p>
  ...
  },
  {
    ...
    body: `
      ...
      <p>For example, a model learns a function <code>ŷ = f<sub>θ</sub>(x)</code> where <code>θ</code> represents all trainable parameters. During training we compare the prediction against known data, <code>L(θ) = (1/N) Σ (f<sub>θ</sub>(xᵢ) − yᵢ)²</code>. Training means changing <code>θ</code> until the loss becomes smaller. After training, we evaluate the function at an input that was not in the original dataset — that is the practical purpose of learning a function.</p>
  ...
  },
  {
    ...
    body: `
      <p>Deep learning follows the same basic pattern as curve fitting and machine learning, but the chosen model is now a neural network. In the PINN Playground the model is a multilayer perceptron, or MLP — a function made from layers of simple operations. Each layer takes numbers in, applies weights and biases, passes the result through an activation function, and sends the output to the next layer: <code>z = W x + b</code>, <code>h = σ(z)</code>.</p>
      <p>After many layers the network becomes a flexible function <code>ŷ = f<sub>θ</sub>(x)</code>, where <code>θ</code> now includes all weights and biases in all layers. A small network may have hundreds of parameters; a large one may have millions or billions.</p>
  ...
  },
  {
    ...
    body: `
      ...
      <p>The model still predicts a function. In the PINN Playground the network maps position to displacement, <code>(x, y) ↦ (u(x, y), v(x, y))</code>. Because the network is differentiable, we can take its derivatives and check whether physics is satisfied.</p>
      <p>Many physical systems are described by differential equations. The equation can often be rearranged into a residual form <code>R(x) = 0</code>. If a neural network predicts a field, we substitute that prediction into the equation; if the prediction satisfies the physics, the residual should be close to zero. The PDE loss is <code>L<sub>PDE</sub> = (1/N) Σ |R(xᵢ)|²</code>. The points <code>xᵢ</code> are called collocation points.</p>
      <p>For 2D solid mechanics, the network predicts displacement; we then differentiate to get strain, apply the material law to get stress, and require equilibrium <code>∇·σ = 0</code> in the interior. Boundary conditions become additional terms in the loss: the bottom is fixed, the top load patch receives traction, the free boundaries should have near-zero traction.</p>
      <p>So the total PINN loss is built from several pieces, <code>L = w<sub>PDE</sub> L<sub>PDE</sub> + w<sub>BC</sub> L<sub>BC</sub> + w<sub>data</sub> L<sub>data</sub></code>. The data term may be absent. In a pure PINN the model trains only from physics residuals and boundary conditions; in a teacher-guided PINN, sparse data from a numerical solution can be added.</p>
  ...
  },
  {
    ...
    body: `
      ...
      <p><strong>Input normalization</strong> maps coordinates into a numerically convenient range, often around <code>[−1, 1]</code>. Networks are sensitive to scale, and even when coordinates are already small, normalization can make activation behavior more stable. In the playground, input normalization helps the MLP use its capacity more effectively across the domain.</p>
      ...
      <p><strong>Dirichlet versus Neumann boundary conditions.</strong> Dirichlet conditions specify the value of the solution directly — direct supervision on the network output. Neumann conditions specify a derivative-related quantity such as traction; the model predicts displacement, but the load condition is imposed through stress, which depends on displacement gradients. That makes the learning signal more indirect, which is one reason a plain PINN may learn a plausible-looking displacement field while still under-predicting stress magnitude.</p>
  ...
  },
  {
    ...
    body: `
      ...
      <p>Because neural networks are differentiable, they can participate naturally in gradient-based optimization. A trained model provides not only a prediction but also derivatives of that prediction with respect to inputs or design variables. That makes PINN-style models attractive for workflows where the goal is not only to solve one case, but to search through a design space.</p>
