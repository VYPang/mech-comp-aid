...
export const TUTORIAL_SECTIONS = [
  ...
  },
  {
    ...
    body: String.raw`
      ...
      <p>What matters most is not only that teacher points add direct displacement guidance, but also <strong>where</strong> that guidance is placed. In this playground, teacher points do not replace physics; they act as sparse anchors that make the optimization problem easier to solve. The most effective placement is an empirical question. Under the current settings, adding points in the interior or on the non-load boundary often reduces the global stress error more efficiently than concentrating the same number of points only on the load patch.</p>
